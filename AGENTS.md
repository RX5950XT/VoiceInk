...
九個分頁：聊天（**專案工作區與終端機併在同一頁**：側欄兩顆鈕切專案／對話（側欄只列專案，終端機在分頁列上），中間有分頁列（每個專案自己一組），右邊是檔案總管／Git／AI 記錄；Ctrl+P 快速開檔、Ctrl+W／Ctrl+Tab 管分頁、檔案樹可用方向鍵走、可多選與拖曳搬檔；編輯器是 Monaco，Git 面板含 worktree）｜CC代理｜額度（＋用量統計）｜AGY 反代｜語音轉文字｜翻譯與 TTS｜系統監控（含使用時長／風扇控制／效能調整）｜HF模型｜設定。
