...
- **插拔隨身碟／手機不輪詢**：`main.js` 對主視窗 `hookWindowMessage(0x0219)`（WM_DEVICECHANGE 廣播，藏起來的視窗也收得到），等 1.5 秒安靜再送 `explorer:devicesChanged`，檔案頁重讀側欄磁碟、站在首頁就重畫卡片。回歸 `e2e-explorer-cdp.js` 的 [G]（對測試視窗丟 WM_DEVICECHANGE）。
- **手機（MTP）是虛擬路徑 `mtp:裝置名稱\儲存空間\…`**（`explorer/mtp.js`，跟 ZIP 同一招：`index.js` 每個入口先問 `mtp.isMtp`）。沒有磁碟代號、fs 碰不到，全走殼層 sidecar（`Portable.cs`：`mtpList`／`mtpCopy`／`mtpDelete`，`IShellItem` 列舉＋`IFileOperation`）。裝置清單從殼層 `NameSpace(17)` 撈非檔案系統項目（`drives.listDevices`，只收 `DEVICE_RE`）。檔名用 `System.FileName`（`SIGDN_NORMALDISPLAY` 會被「隱藏副檔名」吃掉副檔名）；列過的項目記下殼層解析名稱（物件 ID），開檔／複製不用再從裝置一層層列舉（相機資料夾幾百張要 4～8 秒）。**複製／刪除另開一顆 sidecar**（`IFileOperation` 會卡到做完，用常駐那顆圖示和右鍵選單全都等著），進度與同名詢問是 Windows 自己的視窗。只做瀏覽／開檔（先複製到暫存）／複製進出／**永久刪除**（沒有回收筒，renderer 一律問「永久刪除」；裝置與儲存空間本身不給刪）；改名、新增資料夾在 renderer 的 `blockInZip` 和 main 的 `writable` 兩邊擋。資料夾不算大小（要把整棵樹從手機列回來）。改完 C# 要 `npm run build:shell`。回歸 `test-explorer.js`（parseDevices、`mtp.parse`）＋ **`probe-explorer-phone-cdp.js`**（要真的插著手機，沒插就 SKIP；進手機→複製進去→F2 被擋→複製出來→永久刪除，最後確認手機上沒留測試檔）。**測試腳本別用 Bash heredoc 寫**：反斜線會被吃掉，`${dir}\\Download` 變成 `${dir}Download`，看起來像 sidecar 找不到資料夾。
