...
Windows 桌面 AI 工作台。**聊天**、**終端機**、**訂閱額度**、**AGY 反向代理**、**語音轉文字**（檔案轉錄／即時字幕）、**翻譯與 TTS** 放在同一個 App；語音相關功能可完全離線（本地模型、CPU 即時），也能改走雲端 API。
...
同一分頁裡兩個子分頁，共用標題旁的模型選單（要用哪一顆語音／翻譯模型，在這裡直接選，不必跑去設定頁）。
...
| llama.cpp 執行環境（Vulkan） | — | 34 MB | 上面那顆 GPU 語音模型的執行檔；任何有現代顯卡驅動的機器都能跑，不需 CUDA |
...
**要用哪一顆模型，在「語音轉文字」與「翻譯與 TTS」頁面標題旁的下拉直接選**（選了立刻生效，兩頁看到同一份設定）。
...
導覽順序：**聊天（預設）｜終端機｜額度｜AGY反代｜語音轉文字｜翻譯與 TTS｜設定**
