 ...
 *
 * 排版（條列、編號、空行分段）是**兩種模式共用**的第 6 條：使用者要的是貼進輸入框
 * 就已經排好的文字，不是一整行逐字稿；短句那條「維持一行」也寫在同一條裡，
 * 免得模型把一句指令硬斷成三行。
 *
...
function buildSystemPrompt(opts = {}) {
  ...
  const lines = [
    ...
    // ASR 錯字修正：同音字／近音字聽錯是語音輸入最主要的錯誤來源，所以這一條要模型
    // **主動用上下文判斷**，而不是「只有非常確定才改」——寫得太保守時，小模型會照著
    // 錯字原樣抄出來（實測「定選」「因該」都留著）。底線改由第 5 條獨立守住：
    // 意思不能變。範例裡也各放一句錯字，小模型靠範例學得比靠規則快。
    // 整理後與原文的詞級 diff 會被自動學進字典（見 learnPairs），同一個聽錯出現兩次
    // 就由字典直接修，之後連整理模型掛掉都修得到。
    '4. 修正語音辨識聽錯的字：同音、近音、連音造成的錯別字，一律用上下文判斷該是哪個寫法（「定選」→「釘選」、「因該」→「應該」、「在」與「再」、「的得地」、聽錯的人名與專有名詞都算）。某個詞在這個句子裡根本說不通、或整句讀起來不像正常人會講的話，那幾乎就是聽錯了，要改成正確的寫法，不要照著錯的字留下來。',
    rewrite
      ? '5. 意思不能變：只修辨識錯誤，不要改掉他真正想講的事。'
      : '5. 意思不能變：只修辨識錯誤，不要換掉使用者原本的用詞或重寫整句。',
    '6. 排版，不要整段擠成一行：在列項目就每項一行、用「- 」開頭；在報步驟就用「1. 」開頭編號；講到不同主題、或明顯換一段內容時，就空一行分段。只有一句話或一個指令時維持一行，不要硬斷。條列與分段都用真的換行，不要用頓號硬串。',
    '7. 技術名詞、產品名、指令、檔名、程式碼、網址、英文縮寫一律保留原樣，不要翻譯也不要改大小寫。數字與單位照講的寫。',
    // 「輸出語言」是**選字習慣**，不是翻譯指令。以前寫成「輸出語言：繁中」，
    // 使用者講英文時整段會被翻成中文——那是把口述變成翻譯，不是整理。
    `8. 不要翻譯。使用者講什麼語言就輸出什麼語言；需要選字時採用${langName}的用字與標點習慣。`,
    '9. 直接輸出整理後的文字。不要加開場白、不要加引號、不要加標題、不要說明你做了什麼。',
    '',
    // 小模型（0.8B～4B）靠範例學得比靠規則快很多，而這一條管線的本地選項就是小模型
    '範例：',
    '輸入：嗯那個 我想說 我們今天 呃 先把那個登入的部分做完 然後再看時間',
    '輸出：我想我們今天先把登入的部分做完，然後再看時間。',
    '輸入：這個檔案的首頁上面會顯示定選在側邊欄的那些資料夾 你因該看的到',
    '輸出：這個檔案的首頁上面會顯示釘選在側邊欄的那些資料夾，你應該看得到。',
    '輸入：待辦有三個 第一個是修那個登入的 bug 第二個 呃 把設定頁補完 然後第三個是寫測試',
    '輸出：待辦有三個：\n\n- 修登入的 bug\n- 把設定頁補完\n- 寫測試',
