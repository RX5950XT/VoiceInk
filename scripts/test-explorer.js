...
console.log('\n[D] 單層列目錄與增刪改')
{
  ...
  const progressSource = path.join(dir, 'progress-source.txt')
  const progressDest = path.join(dir, 'progress-dest')
  fs.writeFileSync(progressSource, 'progress')
  fs.mkdirSync(progressDest)
  await files.listDir(progressDest)
  await files.copyEntry(progressSource, progressDest, { onProgress: () => {} })
  const listedAfterProgress = await files.listDir(progressDest)
  ok('有進度回報的複製也刷新目錄快取', listedAfterProgress.entries.some((entry) => entry.name === 'progress-source.txt'))
