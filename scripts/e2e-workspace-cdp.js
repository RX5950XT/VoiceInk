...
function seedWorkspacesJson() {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({
      projects: [{ id: PROJECT_ID, name: 'e2e 專案', path: PROJECT_DIR, createdAt: Date.now() }]
    }, null, 2)
  )
}
  ...
  try {
    ...
    const modes = await cdp.eval(`(async () => {
      const heights = () => ({
        ...
        term: document.getElementById('termList').offsetHeight
      })
      const out = {}
    ...
    ok('[A] 切到「專案」時專案與終端機清單都量得到（同一面板）',
      modes.projects.proj > 0 && modes.projects.chat === 0 && modes.projects.term > 0,
      JSON.stringify(modes.projects))
    ok('[A] 切到「對話」時只有對話清單量得到高度',
      modes.chats.chat > 0 && modes.chats.proj === 0 && modes.chats.term === 0,
    ...
    ok('[M] 只有目錄進得來（檔案與不存在的路徑被略過，總數 2）',
      !!dropInfo && dropInfo.total === 2 && Boolean(dropInfo.hitId), JSON.stringify(dropInfo))
    ok('[M] 拖完選中的是新專案',
      await waitInPage(cdp,
    ...
    ok('[M] 重複拖同一夾會略過（總數維持 2）',
      Number(reDropInfo.total) === 2 && String(reDropInfo.toast).includes('略過'), JSON.stringify(reDropInfo))
