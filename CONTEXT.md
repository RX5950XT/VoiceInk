...
- **內容／權限**：右鍵「內容」、指令列「內容」、Alt+Enter → Windows 原生內容視窗（含「安全性」分頁）。
  根因是殼層 sidecar 主執行緒沒跑訊息迴圈（`native/explorer-shell/Program.cs`）；改完要 `npm run build:shell`。
- **原生右鍵選單**本來就是完整的（WinRAR、7-Zip、開啟檔案、傳送到、授與存取權、還原舊版…），實測確認。
- **ZIP**：`explorer/zip.js`＋`zip-ops.js`，點兩下 .zip 走進去、唯讀、詳情看得到內容、複製貼上＝解壓縮、
  「解壓縮到…」「全部解壓縮」。沒有新依賴。
- 「開啟／另存新檔」對話框是別的程式自己跟 Windows 要的，換不掉，不做。
- 測試：`test-explorer-zip.js`（35 條）、`e2e-explorer-zip-cdp.js`（11 條，打包版）。

### 檔案頁工具列三排、回收筒強制釘選、Git 行數分開算（2026-09-25）

- **檔案頁工具列**：第二排只剩 上一頁／下一頁／上一層／重新整理（`#exRefreshBtn`，同 F5）＋路徑＋搜尋；
  篩選、新增、排序、檢視、雙欄、收合詳情搬到第三排 `.ex-cmdrow`（`#exCmdBar` 在左、自己橫捲；整排不可 overflow，篩選面板會被裁）。
- **資源回收筒**：側欄固定，舊存檔藏過的也會回來（`test-explorer.js`「強制釘選」）。
- **Git 行數**：暫存區看 `diff --cached`、變更看 `diff`、新檔自己數行（`test-workspace.js` [U]）；
  檔案樹點改過的檔案預設開檢視變更（`e2e-workspace-cdp.js` [R][V]）。
